...
object RasterLoader {
  ...
  private val loadCache:mutable.Map[String,RasterDataSource] =
    new mutable.HashMap[String,RasterDataSource]()

  def loadRaster(name: String) = {
    loadCache(name) =  Demo.server.run(RasterDataSource(name))
  }
  ...
  def load(name: String):RasterDataSource = {
